...
(()=>{
  const extra={
    en:{title:'Book your tour',subtitle:'Choose your date and enter your contact details.',route:'Your day',pickup:'Hotel pickup & return',cancel:'Free cancellation up to 48h',total:'Total',charged:'Charged securely in EUR',secure:'Secure Stripe payment',instant:'Immediate confirmation',phoneCountry:'Country',phoneNumber:'Phone / WhatsApp (optional)',pickupOptional:'Hotel or accommodation (optional)',pickupHint:'You can confirm your pickup location later.'},
    pt:{title:'Reserve o seu tour',subtitle:'Escolha a data e indique os seus dados de contacto.',route:'O seu dia',pickup:'Recolha e regresso ao alojamento',cancel:'Cancelamento gratuito até 48h',total:'Total',charged:'Cobrado com segurança em EUR',secure:'Pagamento seguro Stripe',instant:'Confirmação imediata',phoneCountry:'País',phoneNumber:'Telefone / WhatsApp (opcional)',pickupOptional:'Hotel ou alojamento (opcional)',pickupHint:'Pode confirmar o local de recolha mais tarde.'},
    es:{title:'Reserva tu tour',subtitle:'Elige la fecha e introduce tus datos de contacto.',route:'Tu día',pickup:'Recogida y regreso al alojamiento',cancel:'Cancelación gratuita hasta 48h',total:'Total',charged:'Cobro seguro en EUR',secure:'Pago seguro con Stripe',instant:'Confirmación inmediata',phoneCountry:'País',phoneNumber:'Teléfono / WhatsApp (opcional)',pickupOptional:'Hotel o alojamiento (opcional)',pickupHint:'Puedes confirmar el lugar de recogida más adelante.'},
    fr:{title:'Réservez votre excursion',subtitle:'Choisissez la date et indiquez vos coordonnées.',route:'Votre journée',pickup:'Prise en charge et retour',cancel:'Annulation gratuite jusqu’à 48 h',total:'Total',charged:'Débité en EUR en toute sécurité',secure:'Paiement Stripe sécurisé',instant:'Confirmation immédiate',phoneCountry:'Pays',phoneNumber:'Téléphone / WhatsApp (facultatif)',pickupOptional:'Hôtel ou hébergement (facultatif)',pickupHint:'Vous pourrez confirmer le lieu de prise en charge plus tard.'},
    de:{title:'Tour buchen',subtitle:'Datum wählen und Kontaktdaten eingeben.',route:'Ihr Tag',pickup:'Abholung und Rückfahrt',cancel:'Kostenlose Stornierung bis 48 Std.',total:'Gesamt',charged:'Sichere Abrechnung in EUR',secure:'Sichere Stripe-Zahlung',instant:'Sofortige Bestätigung',phoneCountry:'Land',phoneNumber:'Telefon / WhatsApp (optional)',pickupOptional:'Hotel oder Unterkunft (optional)',pickupHint:'Den Abholort können Sie später bestätigen.'}
  };
  const countries=[['US','United States','+1'],['CA','Canada','+1'],['GB','United Kingdom','+44'],['PT','Portugal','+351'],['ES','España','+34'],['FR','France','+33'],['DE','Deutschland','+49'],['IT','Italia','+39'],['NL','Nederland','+31'],['BE','Belgique','+32'],['IE','Ireland','+353'],['CH','Schweiz','+41'],['AT','Österreich','+43'],['BR','Brasil','+55'],['AR','Argentina','+54'],['MX','México','+52'],['AU','Australia','+61']];
  const labels=()=>extra[lang()]||extra.en;
  const deviceLocales=()=>Array.from(new Set([...(navigator.languages||[]),navigator.language].filter(Boolean).map(v=>String(v).replace('_','-'))));
  const localeCountry=()=>{for(const raw of deviceLocales()){try{const region=new Intl.Locale(raw).region?.toUpperCase();if(region&&countries.some(c=>c[0]===region))return region}catch{const region=raw.split('-')[1]?.toUpperCase();if(region&&countries.some(c=>c[0]===region))return region}}return'US'};
  const deviceTourLanguage=()=>{for(const raw of deviceLocales()){const code=raw.slice(0,2).toLowerCase();if(['en','es','pt'].includes(code))return code}return'en'};
  ...
  function hydrateForm(form){const country=form.elements.phoneCountry,language=form.elements.language;if(country)country.value=localeCountry();if(language)language.value=deviceTourLanguage();syncPhone(form);setPrice(form);}
      ...
      <section class="booking-modal__form-wrap">
        ...
        <form data-booking-form data-tour-id="${t.id}" novalidate>
          ...
          <div class="booking-grid booking-contact-fields">
            ...
            <label class="booking-field full">${esc(y.pickupOptional)}<input name="pickup" autocomplete="section-booking shipping street-address" placeholder="${esc(y.pickupOptional)}"><small class="booking-field__hint">${esc(y.pickupHint)}</small></label>
