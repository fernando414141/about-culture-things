// Navigation and film controls. Booking validation lives in app.js.
(() => {
 ...
 function translate(){button.setAttribute('aria-label',drawer.hidden?c().menuOpen:c().menuClose);drawer.setAttribute('aria-label',c().menuOpen);drawer.innerHTML=`<a href="#tours">${esc(c().navTours)}</a><a href="#why">${esc(c().navWhy)}</a><a href="#reviews">${esc(c().navReviews)}</a>`;updateVideo()}
 ...
 const video=document.querySelector('.hero-video'),toggle=document.querySelector('.video-toggle');
 function updateVideo(){toggle.textContent=video.paused?c().playVideo:c().pauseVideo;toggle.setAttribute('aria-label',toggle.textContent)}
 toggle.addEventListener('click',()=>{if(video.paused)video.play().catch(updateVideo);else video.pause()});
 video.addEventListener('play',updateVideo);video.addEventListener('pause',updateVideo);
