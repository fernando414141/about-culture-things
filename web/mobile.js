(()=>{
  ...
  drawer.innerHTML=`<div class="mobile-menu-panel">
  ...
  function syncSettings(){
    ...
    if(desktopCurrency&&mobileCurrency)mobileCurrency.value=desktopCurrency.value;
  }
  syncSettings();
  ...
  desktopLang?.addEventListener('change',syncSettings);
  desktopCurrency?.addEventListener('change',syncSettings);
  ...
  function setMenu(open){
    ...
    if(open)drawer.querySelector('a,select,button')?.focus({preventScroll:true});
  }
  ...
  function enhanceForm(form){
    if(!form||form.dataset.mobileEnhanced)return;
    form.dataset.mobileEnhanced='true';
    const date=form.querySelector('input[name="date"]');
    const pickup=form.querySelector('input[name="pickup"]');
    ...
    if(phone){phone.autocomplete='tel';phone.inputMode='tel';phone.enterKeyHint='next'}
    if(requests){requests.autocapitalize='sentences';requests.enterKeyHint='done'}
  }

  const observer=new MutationObserver(()=>document.querySelectorAll('[data-booking-form]').forEach(enhanceForm));
  observer.observe(document.getElementById('dialog-content')||document.body,{childList:true,subtree:true});
