...
app.get('/api/tours',(req,res)=>res.json({tours:TOURS,currency:'EUR',minGuests:2,maxGuests:6,minDate:minDate(),bookingCutoffHour:BOOKING_CUTOFF_HOUR,bookingTimeZone:BOOKING_TIME_ZONE,languages:LANGUAGES,cancellationPolicyHours:48}));
app.get('/api/availability',(req,res)=>{if(!req.query.tourId&&!req.query.date)return res.json({available:true,blockedDates:[],minDate:minDate()});const tour=tourById(req.query.tourId),date=text(req.query.date,10);if(!tour||!validDate(date))return res.status(400).json({error:'Invalid tour or date.'});res.json({available:true,blockedDates:[],minDate:minDate()})});
...
async function createCheckout(req,res){
 try{
  ...
  const p=req.body||{},tour=tourById(p.tourId),persons=Number(p.persons),customerEmail=email(p.email),customerName=text(p.name,120),tourLanguage=text(p.tourLanguage,2),pickup=text(p.pickup,500)||'To be confirmed',attempt=text(p.bookingAttemptId||req.get('Idempotency-Key'),100);
